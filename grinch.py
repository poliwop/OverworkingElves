...
def desiredProd(bigJob):
  ...
  dProd = Elf.maxProd
  if JOB_RATIO > 0:
    prodChange = SANC_PROD_CHANGE**(bigJob/(60.0*JOB_RATIO)) - 1.0
    boostedChange = prodChange*BOOST
...
def getShortestJob(jobsList):
  return getShortestJobAfter(0, jobsList)

def getShortestJobAfter(minDur, jobsList):
  dur = minDur
  if len(jobsList) > dur:
    while jobsList[dur] == []:
      dur += 1
...
def calculateLongJobLength(cut, jobsList):
  ...
  DDotND = 0
  dur = cut + 1
  while dur < len(jobsList):
    sumND += len(jobsList[dur])
    logDDotND += len(jobsList[dur])*math.log(dur)
    dur += 1
  dur = cut
  while dur > 0:
    DDotND += len(jobsList[dur])*dur
    dur -= 1
  numer = logDDotND - math.log(SANC_PROD_CHANGE)*DDotND/60
...
def minDurationForBoundedJobLength(minLen, maxLen, prod, jobsList):
  ...
  maxDur = min(int(math.ceil(maxLen/prod)), len(jobsList) - 1)
  if len(jobsList) > minDur:
    jobs = jobsList[minDur:maxDur + 1]
    dur = minDur
    while dur <= maxDur and jobs[dur - minDur] == []:
      dur += 1
    if dur <= maxDur:
      return dur
  return 0

def averageEndJobLen(jobsList):
  bigJobs = []
  dur = len(jobsList) - 1
  while dur > 0 and len(bigJobs) < WORKFORCE:
    addJobs = min(WORKFORCE - len(bigJobs), len(jobsList[dur]))
    bigJobs.extend([dur]*addJobs)
    dur -= 1
...
# Elf list setup
  ...
  with open(toyFile, 'rb') as f:
    ...
    print("jobs at least 2824 long:")
    print(jobsInJobRange(jobslist, minimum = 2824))
    print("jobs at most 150 long:")
    print(jobsInJobRange(jobslist, maximum = 151))

#    targetJobLen = calculateLongJobLength(CUT, jobslist)
    print('Target Job Length: ' + str(targetJobLen))
...
#      while len(jobslist) > 0 and currentDate < dt.date(2015,5,1):
        if currentDate.day == 1:
          if currentDate.month == 1:
            ...
            print("Minutes in jobs at least 2800 long:")
            bigMins = minutesInJobRange(jobslist, minimum = 2800)
            print(bigMins)
            print("Minutes in jobs at most 150 long:")
            smallMins = minutesInJobRange(jobslist, maximum = 151)
            print(smallMins)
